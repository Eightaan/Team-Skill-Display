...
function Skillinfo:Debug_Message(message, color, message2, message3, message4)
	...
	if not Skillinfo.overlay then
		...
		Skillinfo.stats = {}
		Skillinfo.mod = Skillinfo.overlay:panel():text{name = "mod", x = - (RenderSettings.resolution.x/2.1) + 0.5 * RenderSettings.resolution.x, y = - (RenderSettings.resolution.y/4) + 4.7/9 * RenderSettings.resolution.y, text = "Skill Info", font = Skillinfo.fonttype, font_size = Skillinfo.fontsize, color = Color("00ff04"), layer = 1}
		local pos = 5
		for i=1, num_player_slots do
	...
	DelayedCalls:Add("Skillinfo:Timed_Remove", 10, function()
